  ...
  <div class="layout">
    ...
    <a-table :loading="loading" style="flex: 1; overflow: auto;" :dataSource="tableData" :pagination="false"
      ...
      <a-table-column key="images" title="图片">
        <template #default="{ record, index }">
          <template v-for="(img) in record.images">
            <img class="image" :src="img" @click="startReview(index)"></img>
          </template>
        </template>
      </a-table-column>
    </a-table>
    <div style="display:flex; gap: 16px; align-items: center; justify-content: space-between;">
      ...
      <a-pagination :pageSizeOptions="[10, 20, 50, 100, 1000, 2000, 9999]" :current="formState.page"
        :pageSize="formState.per_page" :total="total" @change="handlePageChange" show-size-changer responsive />
    </div>
